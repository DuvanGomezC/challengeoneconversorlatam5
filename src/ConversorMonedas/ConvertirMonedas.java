...
public class ConvertirMonedas {
	...
	public void ConvertirDolaresPesosColombianos(double valor) {
		double monedaDolar =  valor * 4157.68 ;
		monedaDolar = (double)Math.round(monedaDolar * 100d) /100 ;
		JOptionPane.showMessageDialog(null, "Tienes Dolares " + monedaDolar + " en pesos Colombianos");
	}
	public void ConvertirEurosPesosColombianos(double valor) {
		double monedaEuro = valor * 4574.06 ;
		monedaEuro = (double)Math.round(monedaEuro * 100d) /100 ;
		JOptionPane.showMessageDialog(null, "Tienes Euros " + monedaEuro + " en pesos Colombianos");
	}
	public void ConvertirLibraPesosColombianos(double valor) {
		double monedaLibra = valor * 5345.14;
		monedaLibra = (double)Math.round(monedaLibra * 100d) /100 ;
		JOptionPane.showMessageDialog(null, "Tienes Libras Esterlinas " + monedaLibra + " en pesos Colombianos");
	}
	public void ConvertirYenPesosColombianos(double valor) {
		double monedaYen = valor * 29.41;
		monedaYen = (double)Math.round(monedaYen * 100d) /100 ;
		JOptionPane.showMessageDialog(null, "Tienes Yen Japonés " + monedaYen + " en pesos Colombianos");
	}
	public void ConvertirWonPesosColombianos(double valor) {
		double monedaWon = valor * 3.20;
		monedaWon = (double)Math.round(monedaWon * 100d) /100 ;
		JOptionPane.showMessageDialog(null, "Tienes Won sub-coreano " + monedaWon + " en pesos Colombianos");
	}
